...
import {Node, Path, PathSegment, Relationship, UnboundRelationship, RemoteBlob, InlineBlob} from '../graph-types';
...
const BOLT_VALUE_TYPE_BLOB_INLINE = 0xC5;
const BOLT_VALUE_TYPE_BLOB_REMOTE = 0xC4;
...
class Unpacker {
  _unpackBlob(marker, buffer) {
    if (marker == BOLT_VALUE_TYPE_BLOB_REMOTE || marker == BOLT_VALUE_TYPE_BLOB_INLINE) {
      ...
      var lenBlob = l2[0] << 32 | l2[1] << 16 | l2[2];
      /*
      var lenBlob = parseInt(parseInt(
      ...
      */
      ...
      var mimeTypeCode = parseInt(l2[3]);
      var mime = mimeNames["" + mimeTypeCode];
      ...
      //remote
      if(marker == BOLT_VALUE_TYPE_BLOB_REMOTE) {
        var lenHanlde = buffer.readUInt32();
        var handle = utf8.decode(buffer, lenHanlde);

        return new RemoteBlob(bid, lenBlob, mimeTypeCode, mime, handle);
      }
      else {
        var data = buffer.readSlice(lenBlob);
        return new InlineBlob(bid, lenBlob, mimeTypeCode, mime, data);
      }
    }
